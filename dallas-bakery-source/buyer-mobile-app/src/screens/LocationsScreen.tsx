...
type Props = {
  ...
  userInitials: string;
  /** Opens Help, where a phone call works even with no mail account set up. */
  onRequestLocation: () => void;
};

export function LocationsScreen({ cartCount, locations, onCart, onSelectLocation, onTab, selectedLocationId, userInitials, onRequestLocation }: Props) {
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.addCard}>
          ...
          <PrimaryButton label="REQUEST ANOTHER LOCATION" onPress={onRequestLocation} outline />
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  company: { marginTop: 3, color: colors.rust, fontFamily: fonts.sansMedium, fontSize: 9.4, letterSpacing: 0.5 },
  address: { marginTop: 6, color: colors.muted, fontFamily: fonts.sans, fontSize: 9, lineHeight: 14 },
  active: { color: colors.sage, fontFamily: fonts.sansMedium, fontSize: 9.2, letterSpacing: 0.7 },
  addCard: { marginTop: 16, padding: 17, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.paper },
  addKicker: { color: colors.rust, fontFamily: fonts.sansMedium, fontSize: 9.4, letterSpacing: 1 },
